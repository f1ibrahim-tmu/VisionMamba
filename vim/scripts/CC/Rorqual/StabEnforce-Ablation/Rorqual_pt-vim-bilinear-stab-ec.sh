...
# Feature-StabEnforce: Eigenvalue Clamping only - Bilinear discretization
...
echo "Running Eigenvalue Clamping only with Bilinear discretization, seed $SEED"
...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.run --nproc_per_node=2 --master_port $MASTER_PORT \
    ...
    --output_dir /home/f7ibrahi/links/projects/def-wangcs/f7ibrahi/projects/VisionMamba/output/classification_logs/vim_tiny_bilinear_stab_ec_seed${SEED} \
    --resume /home/f7ibrahi/links/projects/def-wangcs/f7ibrahi/projects/VisionMamba/output/classification_logs/vim_tiny_bilinear_stab_ec_seed${SEED}/checkpoint.pth
