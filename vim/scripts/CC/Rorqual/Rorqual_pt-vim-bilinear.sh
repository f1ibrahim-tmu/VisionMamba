...
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.run --standalone --nproc_per_node=4 --master_port=$MASTER_PORT \
    ...
    --seed $SEED \
    --output_dir "${OUTPUT_ROOT}/classification_logs/vim_tiny_bilinear_seed${SEED}" \
    --resume "${OUTPUT_ROOT}/classification_logs/vim_tiny_bilinear_seed${SEED}/checkpoint.pth"