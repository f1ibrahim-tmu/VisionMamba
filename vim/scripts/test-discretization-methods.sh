...
CUDA_VISIBLE_DEVICES=0 python -m torch.distributed.run --nproc_per_node=1 --master_port=0 ./vim/main.py \
    --model vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2 \
    $COMMON_PARAMS \
    --output_dir "${OUTPUT_ROOT}/classification_logs/test_vim_tiny_zoh"

echo "Testing First Order Hold (FOH)..."
CUDA_VISIBLE_DEVICES=0 python -m torch.distributed.run --nproc_per_node=1 --master_port=0 ./vim/main.pyy \
    --model vim_tiny_patch16_224_bimambav2_foh \
    $COMMON_PARAMS \
    --output_dir "${OUTPUT_ROOT}/classification_logs/test_vim_tiny_foh"

echo "Testing Bilinear (Tustin) Transform..."
CUDA_VISIBLE_DEVICES=0 python -m torch.distributed.run --nproc_per_node=1 --master_port=0 ./vim/main.py \
    --model vim_tiny_patch16_224_bimambav2_bilinear \
    $COMMON_PARAMS \
    --output_dir "${OUTPUT_ROOT}/classification_logs/test_vim_tiny_bilinear"
...
CUDA_VISIBLE_DEVICES=0 python -m torch.distributed.run --nproc_per_node=1 --master_port=0 ./vim/main.py \
    ...
    $COMMON_PARAMS \
    --output_dir "${OUTPUT_ROOT}/classification_logs/test_vim_tiny_highorder"
