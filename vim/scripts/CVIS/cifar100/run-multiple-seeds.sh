...
run_method_seeds "ZOH" "$SCRIPT_DIR/pt-cifar100-vim-zoh.sh"

echo "Running First Order Hold (FOH) with multiple seeds..."
run_method_seeds "FOH" "$SCRIPT_DIR/pt-cifar100-vim-foh.sh"

echo "Running Bilinear (Tustin) Transform with multiple seeds..."
run_method_seeds "Bilinear" "$SCRIPT_DIR/pt-cifar100-vim-bilinear.sh"

echo "Running Polynomial Interpolation with multiple seeds..."
run_method_seeds "Polynomial" "$SCRIPT_DIR/pt-cifar100-vim-poly.sh"

echo "Running Higher-Order Hold with multiple seeds..."
run_method_seeds "HighOrder" "$SCRIPT_DIR/pt-cifar100-vim-highorder.sh"

echo "Running Runge-Kutta 4th Order (RK4) with multiple seeds..."
run_method_seeds "RK4" "$SCRIPT_DIR/pt-cifar100-vim-rk4.sh"
