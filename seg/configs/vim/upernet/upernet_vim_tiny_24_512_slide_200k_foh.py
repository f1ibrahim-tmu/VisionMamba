# --------------------------------------------------------
# Vision Mamba Segmentation with First Order Hold (FOH) Discretization
# --------------------------------------------------------'
_base_ = [
...
model = dict(
    backbone=dict(
        ...
        if_divide_out=True,
        if_cls_token=False,
        discretization_method='foh',  # First Order Hold discretization
        ssm_cfg=dict(dt_min=0.0005, dt_max=0.02, dt_scale=0.25),  # Stricter dt range for FOH stability
    ),
    decode_head=dict(
