...
python test.py ${SEG_CONFIG} ${TRAINED_CKPT} --eval mIoU \
    --options model.backbone.if_bimamba=True model.backbone.bimamba_type=v2 optim_wrapper.optimizer.lr=1e-5 model.backbone.use_residual_as_feature=True model.backbone.last_layer_process=add optim_wrapper.paramwise_cfg.layer_decay_rate=0.95