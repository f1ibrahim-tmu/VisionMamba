...
SEG_CONFIG=configs/vim/upernet/upernet_vim_tiny_24_512_slide_60k_poly.py
PRETRAIN_CKPT=/path/to/pretrained_ckpts/pretrained-vim-t.pth

python3 -m torch.distributed.launch --nproc_per_node=4 --nnodes=${WORLD_SIZE:-1} --node_rank=${RANK:-0} --master_addr=${MASTER_ADDR:-localhost} --master_port=10298 \
