#!/bin/bash
# Polynomial Interpolation discretization for Vision Mamba segmentation on ADE20K
#
# Usage: ./ft_vim_tiny_upernet_poly.sh [DATASET_PATH]
...
PRETRAIN_CKPT=/home/f7ibrahi/links/projects/def-wangcs/f7ibrahi/projects/VisionMamba/output/classification_logs/vim_tiny_poly/best_checkpoint.pth

# 3. Resume Logic
# MMEngine saves checkpoints as latest.pth, iter_*.pth, or custom names
WORK_DIR=./output/segmentation_logs/vim_tiny_rorqual_vimseg_upernet_poly
RESUME_ARG=""
CHECKPOINT_PATH=""
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.run --standalone --nproc_per_node=4 --master_port=$MASTER_PORT \
    ...
    # --wandb-run-name vim_tiny_upernet_poly_cc-rorqual \
    # --wandb-tags segmentation poly cc-rorqual \
