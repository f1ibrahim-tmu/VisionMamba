...
PRETRAIN_CKPT=/home/f7ibrahi/links/projects/def-wangcs/f7ibrahi/projects/VisionMamba/output/classification_logs/vim_tiny_highorder/best_checkpoint.pth

# 3. Resume Logic
# MMEngine saves checkpoints as latest.pth, iter_*.pth, or custom names
WORK_DIR=./output/segmentation_logs/vim_tiny_rorqual_vimseg_upernet_highorder
RESUME_ARG=""
CHECKPOINT_PATH=""
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.run --standalone --nproc_per_node=4 --master_port=$MASTER_PORT \
    ...
    # --wandb-run-name vim_tiny_upernet_highorder_cc-rorqual \
    # --wandb-tags segmentation highorder cc-rorqual \
