...
--use_env train.py --launcher pytorch \
    ${SEG_CONFIG} \
    --seed 0 --work-dir "${OUTPUT_ROOT}/segmentation_logs/vimseg-t" --deterministic \
    --options model.backbone.pretrained=${PRETRAIN_CKPT} model.backbone.init_backward_from_forward=${INIT_BACKWARD_FROM_FORWARD} model.backbone.if_bimamba=True model.backbone.bimamba_type=v2 optim_wrapper.optimizer.lr=1e-5 optim_wrapper.optimizer.weight_decay=0.01 train_cfg.max_iters=200000 