...
#define WARP_THREADS HIPCUB_WARP_THREADS
    ...
    WarpReverseScan()
        : lane_id(cub::LaneId()), warp_id(IS_ARCH_WARP ? 0 : (lane_id / LOGICAL_WARP_THREADS)), member_mask(cub::WarpMask<LOGICAL_WARP_THREADS>(warp_id))
    {
        if (!IS_ARCH_WARP)
        {
        ...
        else
        {
            ...
            if (linear_tid < RAKING_THREADS)
            {
                ...
                T downsweep_postfix = linear_tid == RAKING_THREADS - 1
                                          ? block_postfix
                                          : scan_op(block_postfix, exclusive_partial);
