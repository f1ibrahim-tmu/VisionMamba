...
const TrainingLogAnalyzer = () => {
    ...
    for (const line of lines) {
      try {
        ...
        data.push({
          ...
          test_acc1: parsed.test_acc1,
          test_acc5: parsed.test_acc5,
          train_lr: parsed.train_lr,
          n_parameters: parsed.n_parameters,
        });
      } catch (e) {
    ...
    (event) => {
      ...
      files.forEach((file) => {
        ...
        reader.onload = (e) => {
          ...
          const dataset = parseLogFile(content, file.name);
          setDatasets((prev) => [...prev, dataset]);
          setSelectedDatasets((prev) => new Set([...prev, dataset.id]));
        };
  ...
  // Load initial data from the provided log
  React.useEffect(() => {
    const initialData = `{"train_lr": 9.9999999999991e-07, "train_loss": 6.922781705689563, "test_loss": 6.9021368203339755, "test_acc1": 0.20600001268386842, "test_acc5": 0.936000042629242, "epoch": 0, "n_parameters": 7148008}
...
{"train_lr": 0.0001207375862658481, "train_loss": 5.184730062882106, "test_loss": 2.7569607231352062, "test_acc1": 42.15200180664063, "test_acc5": 68.25800258789063, "epoch": 38, "n_parameters": 7148008}
...
{"train_lr": 0.00010858217907886544, "train_loss": 4.908938155507298, "test_loss": 2.362476044230991, "test_acc1": 48.9140025390625, "test_acc5": 74.49200419921875, "epoch": 75, "n_parameters": 7148008}`;
    ...
    const initialDataset = parseLogFile(initialData, "log.txt");
    setDatasets([initialDataset]);
    setSelectedDatasets(new Set([initialDataset.id]));
  }, []);
  ...
  const exportToCSV = () => {
    ...
    let csvContent =
      "Dataset,Epoch,Train Loss,Test Loss,Test Acc1,Test Acc5,Train LR,N Parameters\n";

    datasets.forEach((dataset) => {
      dataset.data.forEach((row) => {
        csvContent += `${dataset.name},${row.epoch},${row.train_loss},${row.test_loss},${row.test_acc1},${row.test_acc5},${row.train_lr},${row.n_parameters}\n`;
      });
    });
  ...
  const getVisibleDatasets = () => {
    return datasets.filter((dataset) => dataset.visible);
  };
  ...
  const combineDataForChart = () => {
    ...
    const maxEpochs = Math.max(...visibleDatasets.map((d) => d.data.length));
    const combined = [];
    ...
    for (let epoch = 0; epoch < maxEpochs; epoch++) {
      ...
      visibleDatasets.forEach((dataset) => {
        const dataPoint = dataset.data[epoch];
        if (dataPoint) {
          point[`${dataset.name}_train_loss`] = dataPoint.train_loss;
          point[`${dataset.name}_test_loss`] = dataPoint.test_loss;
          point[`${dataset.name}_test_acc1`] = dataPoint.test_acc1;
          point[`${dataset.name}_test_acc5`] = dataPoint.test_acc5;
        }
      });
  ...
  const renderChart = (title, dataKeys, yAxisLabel) => {
    const data = combineDataForChart();
    const visibleDatasets = getVisibleDatasets();
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={data}>
            ...
            <XAxis dataKey="epoch" stroke="#666" fontSize={12} />
            <YAxis
              stroke="#666"
              fontSize={12}
            ...
            {visibleDatasets.map((dataset, index) =>
              dataKeys.map((key) => (
                <Line
                  key={`${dataset.id}_${key}`}
                  type="monotone"
                  dataKey={`${dataset.name}_${key}`}
                  stroke={dataset.color}
                  strokeWidth={2}
                  dot={false}
                  name={`${dataset.name} ${key.replace("_", " ")}`}
                  strokeDasharray={key.includes("test") ? "5 5" : "0"}
                />
              ))
            )}
          </LineChart>
              ...
              {datasets.map((dataset) =>
                dataset.data.map((row, index) => (
                  ...
                  >
                    <td
                      className="px-4 py-2"
                      style={{ color: dataset.color, fontWeight: "bold" }}
                    >
                      {dataset.name}
                    </td>
                    <td className="px-4 py-2">{row.epoch}</td>
                    <td className="px-4 py-2">{row.train_loss.toFixed(4)}</td>
                    <td className="px-4 py-2">{row.test_loss.toFixed(4)}</td>
                    <td className="px-4 py-2">{row.test_acc1.toFixed(2)}</td>
                    <td className="px-4 py-2">{row.test_acc5.toFixed(2)}</td>
                    <td className="px-4 py-2">
                      {row.train_lr.toExponential(3)}
                    </td>
                    <td className="px-4 py-2">
                      {row.n_parameters.toLocaleString()}
                    </td>
                  </tr>
                ))
              )}
  ...
  const handleColorChange = (id, newColor) => {
    setDatasets((prev) =>
      prev.map((dataset) =>
        dataset.id === id ? { ...dataset, color: newColor } : dataset
      )
    );
  };
                ...
                >
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      value={dataset.color}
                      onChange={(e) =>
                        handleColorChange(dataset.id, e.target.value)
                      }
        ...
        {datasets.length === 0 ? (
          <div className="bg-white rounded-lg shadow-lg p-12 text-center">
            ...
            <p className="text-gray-500 mb-6">
              Upload training log files to get started
            </p>
            <button
              onClick={() => fileInputRef.current?.click()}
